...
program
    ...
    .option('--parallel [parallel]', 'Aggressively paralellize tasks', true)
    .action((options) => {
        ...
        console.log(config);
        build({ projectDirectory: options.projectDir, outputDirectory, cacheDirectory, parallel: options.parallel, config });
    });
